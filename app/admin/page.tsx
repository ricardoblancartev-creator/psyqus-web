...
export default function AdminDashboard() {
  ...
  // 1. CARGAR PROSPECTOS (TABLA DE VENTAS)
  const fetchProspectos = async () => {
    const { data } = await supabase.from('prospectos').select('*').order('created_at', { ascending: false });
    if (data) setProspectos(data);
  };

  // 2. CARGAR INCIDENCIAS (BUZÓN Y ALERTAS)
  const fetchIncidencias = async () => {
    const { data } = await supabase.from('incidencias').select('*').order('created_at', { ascending: false });
    if (data) setIncidencias(data);
  };

  // 3. CALCULAR PROMEDIOS REALES PARA EL RADAR
  const fetchRadarStats = async () => {
    const { data } = await supabase.from('resultados_encuesta').select('modulo_1_score, modulo_2_score, modulo_3_score, modulo_4_score, modulo_5_score, modulo_6_score');
    
    if (data && data.length > 0) {
      const n = data.length;
      const averages = [
        data.reduce((acc, curr) => acc + curr.modulo_1_score, 0) / n,
        data.reduce((acc, curr) => acc + curr.modulo_2_score, 0) / n,
        data.reduce((acc, curr) => acc + curr.modulo_3_score, 0) / n,
        data.reduce((acc, curr) => acc + curr.modulo_4_score, 0) / n,
        data.reduce((acc, curr) => acc + curr.modulo_5_score, 0) / n,
        data.reduce((acc, curr) => acc + curr.modulo_6_score, 0) / n,
      ];
      setRadarScores(averages);
    }
  };
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-slate-200 p-6 lg:p-12 relative font-sans">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {/* COLUMNA IZQUIERDA: VENTAS Y RADAR */}
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-slate-900/30 border border-slate-800 rounded-[2.5rem] p-8 backdrop-blur-sm">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-[10px] font-bold uppercase tracking-widest">
                  ...
                  <tbody>
                    {prospectos.map((p, i) => (
                      <tr key={i} className="border-b border-slate-800/50 hover:bg-cyan-500/5 transition-all text-slate-300">
            ...
            <div className="bg-slate-900/30 border border-slate-800 rounded-[2.5rem] p-8">
              ...
              <div className="max-w-md mx-auto bg-black/20 rounded-full p-6 shadow-2xl border border-slate-800">
                <RadarBienestar scores={radarScores} />
              </div>
            </div>
          </div>

          {/* COLUMNA DERECHA: BUZÓN Y ALERTAS */}
          <aside className="space-y-8">
            <section className="bg-slate-900/30 border border-slate-800 rounded-[2.5rem] p-8 h-fit">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xs font-black text-white uppercase tracking-widest italic">Incidentes y Alertas</h3>
                <span className="px-2 py-1 bg-purple-500/20 text-purple-400 text-[8px] rounded-full font-bold">{incidencias.length} TOTAL</span>
              </div>
              <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                {incidencias.map((item, i) => (
                  <div key={i} className={`p-5 rounded-2xl border ${item.tipo === 'SISTEMA' ? 'bg-red-500/10 border-red-500/30' : 'bg-black/40 border-slate-800'}`}>
                    ...
                    <p className="text-slate-300 text-xs italic leading-relaxed">"{item.mensaje}"</p>
                  </div>
                ))}
              </div>
            </section>
